...
function template_main()
{
	...
	/* Kategorilerdeki her kategori şunlardan oluşur:
	id, href, link, name, is_collapsed (daraltılmış mı?), can_collapse (daraltılmışsa sorun yok mu?),
	new (yeni mi?), collapse_href (daraltmak/genişletmek için href), collapse_image (yukarı/aşağı görüntü),
	ve panolar. (aşağıya bakınız.) */
		...
		<div class="topic-item">
			...
			<div class="topic-item-content">
				...
				<div class="topic-item-details">
					<div class="topic-item-poster">Yazan: ', $board['last_post']['member']['link'], ',</div>
	...
	if (!empty($board['children']))
	{
		...
		foreach ($board['children'] as $child)
		{
			if (!$child['is_redirect'])
				$child['link'] = '' . ($child['new'] ? '<a href="' . $scripturl . '?action=unread;board=' . $child['id'] . '" title="' . $txt['new_posts'] . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')" class="new_posts"><i class="fas fa-bowling-ball"></i></a> ' : '') . '<a href="' . $child['href'] . '" ' . ($child['new'] ? 'class="board_new_posts" ' : '') . 'title="' . ($child['new'] ? $txt['new_posts'] : $txt['old_posts']) . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')">' . $child['name'] . '</a>';
			else
