...
function getCharacterFactionById($factionId)
{
	$oceanPdo = connectToOceanRP();
	$stmt2 = $oceanPdo->prepare('SELECT name FROM factions WHERE id = ?');
	$stmt2->execute([$factionId]);
	$characterData = $stmt2->fetchAll(PDO::FETCH_OBJ);

	if ($characterData)
		return $characterData[0];
	else
		return 'Birlik bulunmuyor!';
}
...
function getCharacterDataById($id)
{
	...
	$oceanPdo = connectToOceanRP();
	$stmt2 = $oceanPdo->prepare(query: 'SELECT age, charactername, weight, height, faction_id, skin FROM characters WHERE account = ?');
...
{
	global $context, $settings, $options, $txt, $scripturl, $modSettings, $gameCharacterID, $smcFunc, $user_info;
	...
	if (!$message['member']['is_guest']) {
		...
		if (!isset($context['disabled_fields']['posts']))
			echo '
								<li class="postcount">', $txt['member_postcount'], '<span> ', $message['member']['posts'], '</span></li>';
	...
	$character = getCharacterDataById($message['member']['id']);
	...
	if ($character)
		echo '<img class="character-photo" src="Themes\ParsRoleplay\uploads\Skinid' . $character->skin . '.png" alt="' . $character->charactername . '">';
	else
		echo '<img class="character-photo" src="avatars\default.png" alt="Default Skin">';
	...
	if ($character) {
		echo '<h3>' . $character->charactername . '</h3>
						<div class="detail">
                        	<span>Boy: ' . $character->height . '</span>
                        	<span>Kilo: ' . $character->weight . '</span>
                        	<span>Yaş: ' . $character->age . '</span>
						</div>
						<div class="detail faction">
                            <span><i class="fa-solid fa-globe"></i> ' . getCharacterFactionById($character->faction_id)->name . '</span>
						</div>';
	} else {
		echo '<h3 style="font-size: 12px;">Karakterini Bağlamamış</h3>
                <div class="detail">
                    <span>Boy: N/A</span>
                    <span>Kilo: N/A</span>
                    <span>Yaş: N/A</span>
                ...
                <div class="detail faction">
                    <span><i class="fa-solid fa-globe"></i> N/A</span>
                </div>';
	}
	echo '<hr>
				</div>
	...
	// Bu gönderi düzenlenmişse “<< Son Düzenleme: Kişiye Göre Zaman >>” ifadesini gösterin. Ancak değiştirilmemiş olsa bile div'e ihtiyacımız var!
	// Çünkü AJAX aracılığıyla içine ekleme yapıyoruz ve kullanıcıların istedikleri yere koyabilmeleri için istedikleri zaman hareket ettirmelerini engellemek istemiyoruz.
	...
	// Ekler olduğunu varsayarsak...
	if (!empty($message['attachment'])) {
		$last_approved_state = 1;
		// Gerçekten gösterecek bir şeyimiz olmadığı sürece div'in çıktısını almayın...
		$div_output = false;

		foreach ($message['attachment'] as $attachment) {
			// Bu ekin burada gösterilmemesini istiyor muyuz?
			...
			// Onaylanmamış ekler için özel bir kutu göster...
