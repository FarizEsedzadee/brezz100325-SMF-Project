...
function template_main()
{
    ...
    if (!empty($context['members']))
    {
        ...
        {
            echo '
            <div class="member-card" onclick="window.location.href=\'', $scripturl, '?action=profile;u=', $member['id'], '\'">
                <div class="member-avatar">
                    <img src="', !empty($member['avatar']['href']) ? $member['avatar']['href'] : $settings['images_url'] . '/default_avatar.png', '" alt="', $member['name'], '">
                </div>
                <div class="member-name">', $member['link'], '</div>
                <div class="member-group">', empty($member['group']) ? $member['post_group'] : $member['group'], '</div>
                <div class="member-registered">Kayıt Tarihi: ', $member['registered_date'], '</div>
            </div>';
        }
    }
    else
