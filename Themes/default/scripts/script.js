...
function reqOverlayDiv(desktopURL, sHeader, sIcon) {
	...
	if (typeof (sIcon) == 'string' && sIcon.match(/\.(gif|png|jpe?g|svg|bmp|tiff)$/) != null)
		containerOptions = {
			...
			icon: smf_images_url + '/' + sIcon
		};
	else
		containerOptions = {
			heading: sHeader,
			content: sAjax_indicator,
			icon_class: 'main_icons ' + (typeof (sIcon) != 'string' ? 'help' : sIcon)
		};
...
smc_PopupMenu.prototype.add = function (sItem, sUrl) {
	var $menu = $('#' + sItem + '_menu'),
		$item = $('#' + sItem + '_menu_top');
	if ($item.length == 0)
		return;

	this.opt.menus[sItem] = {
		open: false,
		loaded: false,
		sUrl: sUrl,
		itemObj: $item,
...
smc_Popup.prototype.show = function () {
    ...
    // Gösterilecek div'i oluşturun
    $('body').append(
        '<div id="' + this.popup_id + '" class="login_popup_container"><div class="' + popup_class + '"><div class="catbg login_popup_heading"><i class="fa-solid fa-arrow-right-to-bracket"></i>' + ' ' + this.opt.heading + '</div><div class="popup_content">' + this.opt.content + '</div></div></div>'
    );
    ...
    // Kaçışta veya fare tıklamasında gizlemeyi tetikle
    ...
    $('#' + this.popup_id).find('.hide_popup').click(function () {
	...
	// Mozilla text range wrap.
		...
		if (oTextHandle.setSelectionRange) {
			var goForward = is_opera ? text1.match(/\n/g).length : 0,
				goForwardAll = is_opera ? (text1 + text2).match(/\n/g).length : 0;
...
function generateDays(offset) {
	...
	var days = 0,
		selected = 0;
	var dayElement = document.getElementById("day" + offset),
...
smc_preview_post.prototype.onDocSent = function (XMLDoc) {
	...
	var ignored_replies = new Array(),
		ignoring;
	var newPosts = XMLDoc.getElementsByTagName('smf')[0].getElementsByTagName('new_posts')[0] ? XMLDoc.getElementsByTagName('smf')[0].getElementsByTagName('new_posts')[0].getElementsByTagName('post') : {
		length: 0
	};
