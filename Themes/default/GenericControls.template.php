...
/**
 * Bu şablon bir doğrulama formu görüntüler
 *
 * @param int|string $verify_id Doğrulama kontrol kimliği
 * @param string $display_type Hangi tür gösterilecek. Yalnızca bir doğrulama seçeneğini göstermek için 'tek' veya tümünü göstermek için 'tümü' olabilir
 * @param bool $reset Dahili izleme sayacının sıfırlanıp sıfırlanmayacağı
 * @return bool Gösterilecek başka bir şey yoksa false, $display_type 'single' ise true, aksi halde nothing
 */
function template_control_verification($verify_id, $display_type = 'all', $reset = false)
{
	...
	for ($i = 0; $i < $total_items; $i++)
	{
		...
		// Boş alanı göster, ancak yalnızca bir alanımız varsa ve bu ilk seferse.
		...
		{
			// Bu soru soru dizisinin neresinde?
		...
		// Sadece bir tanesini gösteriyor olsaydık ve bunu yapsaydık, kırılırdı.
		if ($display_type == 'single' && $verify_context['tracking'] == $i)
			break;
	}
